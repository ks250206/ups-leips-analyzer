...
function HelpWindow() {
  ...
  return (
    <div className="h-full overflow-auto bg-white p-4 text-sm text-slate-700">
      ...
      <div className="mt-3 flex flex-wrap gap-1" role="tablist" aria-label="Help sections">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            type="button"
            ...
            className={
              activeTab.id === tab.id
                ? "rounded border border-slate-900 bg-slate-900 px-2 py-1 text-xs font-semibold text-white"
                : "rounded border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50"
            }
...
function helpTitle(locale: UserLocale): string {
  return locale === "en-US" ? "UPS-LEIPS Analyzer Help" : "UPS-LEIPS Analyzer ヘルプ";
}
