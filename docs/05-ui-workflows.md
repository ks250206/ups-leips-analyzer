...
- Display language is a per-user setting stored in `localStorage`, not in Project or Catalog state. `Setting > Language` supports `ja-JP` and `en-US`, defaults to `ja-JP`, and currently localizes Sample Info labels/placeholders and the Help window body.
...
- Windows menuは選択したwindowを最前面に移動する`Bring to front`と、window自体の座標を変えずにworkspace viewportを動かして画面中央へ持ってくる`Go to position`を分ける。
- Help menuはalertではなく専用Help windowをtoggle表示する。Help windowはOverview/Data/UPS/LEIPS/Band/REELS/Plot/Shortcutsのタブ構成にし、`ja-JP`では日本語、`en-US`では英語で解析手順を表示する。Windows menuにもHelpを表示する。
...
Projects > Save as ...、Projects > Rename Project、Projects > Delete projectはmodalで確認する。Rename Projectは現在ProjectのIDを維持して名前だけ変更し、保存済みProjectなら同じrecordへ保存する。Projects > Save Projectは未保存New ProjectではSave as ...と同じmodal flowにする。Projects > Load Projectはmodal内にProject一覧を表示する。Save as ...で既存Projectと同じ名前を指定した場合は同名Projectを上書きし、Recentに重複を作らない。
